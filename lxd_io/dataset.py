...
class Dataset:
    ...
    def __init__(
        ...
        if self._dataset_id is None or self._major_version_no is None or self._minor_version_no is None:
            self._read_dataset_info_from_folder_name()

        logger.debug("Read dataset {}-{} from: {}", self._dataset_id, self.version, self._dataset_dir)

        # Load scale factor
        self._background_image_scale_factor = background_image_scale_factor
        if self._background_image_scale_factor is None:
            self._background_image_scale_factor = self._load_background_image_scale_factor()
    ...
    def _load_background_image_scale_factor(self) -> float:
        ...
        if self._dataset_id is None:
            logger.warning("Since dataset_id is None, background_image_scale_factor is assumed to be 1.0")
            return 1.0

        scale_factor_json = Path(__file__).parent / "background_image_scale_factors.json"
        ...
        if self._dataset_id in scale_factor_dict:
            ...
            if version_str in scale_factor_dict[self._dataset_id]:
                background_scale_factor = scale_factor_dict[self._dataset_id][version_str]

        if background_scale_factor is None:
            logger.warning("Dataset id {} could not be found in {}. Scale factor is set to 1.0", self._dataset_id, scale_factor_json)
            return 1.0
    ...
    def _explore_data_dir(self) -> None:
        ...
        def find_files_by_recording_ids(data_dir: Path, suffix: str | list[str]) -> dict:
        ...
        self._recording_meta_files = find_files_by_recording_ids(self._data_dir, self.RECORDING_META_FILE_SUFFIX)
        self._tracks_meta_files = find_files_by_recording_ids(self._data_dir, self.TRACKS_META_FILE_SUFFIX)
        self._tracks_files = find_files_by_recording_ids(self._data_dir, self.TRACKS_FILE_SUFFIX)
        self._background_image_files = find_files_by_recording_ids(self._data_dir, self.BACKGROUND_IMAGE_FILE_SUFFIX)
        ...
        for recording_id in all_potential_recording_ids:
            ...
            if recording_id in self._recording_meta_files and recording_id in self._tracks_meta_files and recording_id in self._tracks_files and recording_id in self._background_image_files:
                self._recording_ids.append(recording_id)

                # Get location id
                recording_meta =  pd.read_csv(self._recording_meta_files[recording_id]).to_dict(orient="records")[0]
        ...
        self._recordings_at_location = {key: sorted(self._recordings_at_location[key]) for key in sorted(self._recordings_at_location.keys())}

        logger.debug("Found complete data for {} recordings:\n{}", len(self._recording_ids ), self._recording_ids)
    ...
    def _find_opendrive_maps(self, opendrive_dir: Path) -> None:
        ...
        if not opendrive_dir.exists() or not opendrive_dir.is_dir():
            logger.warning("OpenDrive map directory does not exist at {}.", opendrive_dir)
            return
    ...
    def get_track_batches(self, track_batch_size: int, recording_ids: list[int] | None = None) -> list[list[int]]:
        ...
        for recording_id in self._recording_ids:
            if recording_ids is not None and recording_id not in recording_ids:
                continue
            track_batches.extend(self.get_recording(recording_id).get_track_batches(track_batch_size))
